...
<template>
  <PageLayout title="Mi Dashboard" description="Etapas de trazabilidad asignadas a ti">
    <template #actions>
    ...
    <!-- Empty current tab -->
    <div v-else-if="visibleStages.length === 0" class="flex flex-col items-center justify-center py-16 text-slate-500">
    ...
    <div v-else class="flex gap-4 overflow-x-auto pb-2">
      <div v-for="col in columns" :key="col.key" class="shrink-0 w-72 flex flex-col gap-3">
        ...
        <div v-for="stage in stagesForColumn(col.key)" :key="stage.id"
          ...
          <!-- Stage name -->
          <h3 class="font-semibold text-white text-sm leading-snug mb-2 group-hover:text-indigo-300 transition-colors">
            {{ stage.name }}
          </h3>
  ...
  <!-- ── Modal ──────────────────────────────────────────────────────────────── -->
  <Teleport to="body">
    <div v-if="activeModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm"
        ...
        <div class="flex items-start justify-between gap-4 p-5 border-b border-slate-800">
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 mb-1 flex-wrap">
              ...
              <span class="text-xs px-2 py-0.5 rounded-full font-medium" :class="stageStatusClass[activeModal.status]">
            ...
            <div class="space-y-2">
              <div v-for="doc in activeModal.documents" :key="doc.id"
                ...
                <div v-if="activeDoc?.id === doc.id" class="border-t border-slate-800 p-4 bg-slate-950">
                  ...
                  <pre v-else class="text-xs text-slate-300 whitespace-pre-wrap font-mono leading-relaxed">{{
                    activeDoc.content || '(sin contenido)' }}</pre>
                </div>
              </div>
            </div>
          </div>
