...
<template>
  <div
    class="min-h-screen bg-gradient-to-br from-base-300 via-base-300 to-indigo-950 flex items-center justify-center p-4">
    <div class="w-full max-w-md">
      ...
      <div class="flex justify-center mb-8">
        <div class="flex items-center gap-3">
          ...
          <span class="text-base-content text-lg font-semibold tracking-wide">Agent Manager</span>
        </div>
      </div>
      ...
      <!-- Authorization card -->
      <div v-else class="bg-base-100/5 backdrop-blur border border-white/10 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div class="px-8 pt-8 pb-6 border-b border-white/10">
          <div class="flex items-center justify-between mb-4">
            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                <p class="text-base-content font-semibold text-sm">{{ clientName }}</p>
                <p class="text-base-content/60 text-xs">is requesting access</p>
              </div>
            </div>
            <!-- Arrow -->
            ...
            <div class="flex items-center gap-3">
              <div>
                <p class="text-base-content font-semibold text-sm text-right">Agent Manager</p>
                <p class="text-base-content/60 text-xs text-right">MCP Server</p>
              </div>
              <div
          ...
          <h2 class="text-base-content text-lg font-semibold mb-1">Authorize Access</h2>
          <p class="text-base-content/60 text-sm">
            Sign in to grant <span class="text-base-content font-medium">{{ clientName }}</span> access to your MCP tools.
          </p>
        </div>

        <!-- Permissions -->
        <div class="px-8 py-5 border-b border-white/10">
          <p class="text-xs font-semibold text-base-content/50 uppercase tracking-wider mb-3">Requested Permissions</p>
          <ul class="space-y-2">
            <li v-for="s in scopes" :key="s.key" class="flex items-start gap-2.5">
              ...
              <span class="text-base-content text-sm">{{ s.label }}</span>
            </li>
          </ul>
        </div>
        ...
        <div class="px-8 py-6">
          ...
          <div v-if="azureLoading" class="flex flex-col items-center justify-center py-6 gap-3">
            ...
            <p class="text-sm text-base-content/60">Completing Microsoft sign-in...</p>
          </div>

          <template v-else>
            <!-- Sign in with Microsoft -->
            <button type="button" @click="loginWithAzure"
              class="w-full flex items-center justify-center gap-3 border border-white/10 hover:bg-base-100/5 text-base-content hover:text-base-content font-medium py-2.5 rounded-lg transition-colors text-sm mb-4">
            ...
            <div class="flex items-center gap-3 mb-4">
              ...
              <span class="text-xs text-base-content/50">or continue with username</span>
              <div class="flex-1 h-px bg-base-100/10"></div>
            </div>

            <p class="text-xs font-semibold text-base-content/50 uppercase tracking-wider mb-4">Your Credentials</p>

            <div class="space-y-3">
              <div>
                <label class="block text-xs font-medium text-base-content/60 mb-1.5">Username or Email</label>
                <input v-model="username" type="text" autocomplete="username" placeholder="Enter your username"
                  class="w-full px-3 py-2.5 rounded-lg bg-base-100/5 border border-white/10 text-base-content text-sm placeholder:text-base-content/40 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent" />
              </div>
              <div>
                <label class="block text-xs font-medium text-base-content/60 mb-1.5">Password</label>
            ...
            <div class="flex gap-3 mt-6">
              <button :disabled="loading"
                class="flex-1 px-4 py-2.5 rounded-lg border border-white/10 text-sm font-medium text-base-content hover:bg-base-100/5 hover:text-base-content transition-colors disabled:opacity-50"
            ...
            <p class="text-center text-xs text-base-content/50 mt-5">
      ...
      <!-- Footer -->
      <p class="text-center text-base-content/40 text-xs mt-6">
