...
<template>
    <PageLayout title="Skills" description="Bloques de instrucciones reutilizables en markdown">
      ...
      <div class="flex flex-1 min-h-0">
        <!-- Skill list -->
        <div class="w-80 shrink-0 border-r border-base-300/60 flex flex-col min-h-0">
          ...
          <div v-else-if="skills.length === 0" class="flex flex-col items-center justify-center py-16 px-6 text-center">
            ...
            <p class="text-sm font-medium text-base-content mb-1">Sin skills todavía</p>
            <p class="text-xs text-base-content/50">Crea tu primer skill para empezar</p>
          </div>

          <div v-else class="flex-1 overflow-y-auto py-3">
            <!-- Active skills -->
            <div v-if="activeSkills.length > 0">
              <p class="px-4 pb-1 text-xs font-semibold text-base-content/50 uppercase tracking-wider">Activos</p>
              <button
                v-for="skill in activeSkills"
                :key="skill.id"
                class="w-full text-left px-4 py-3 hover:bg-base-200/60 transition-colors border-b border-base-300/40 last:border-0"
                :class="selectedSkill?.id === skill.id ? 'bg-base-200/80 border-l-2 border-l-fuchsia-500' : ''"
                @click="selectedSkill = skill">
                <div class="flex items-center justify-between gap-2">
                  <div class="min-w-0">
                    <p class="text-sm font-medium text-base-content truncate">{{ skill.name }}</p>
                    <p class="text-xs text-base-content/50 font-mono truncate">{{ skill.slug }}</p>
                  </div>
            ...
            <div v-if="inactiveSkills.length > 0" class="mt-2">
              ...
              <button
                ...
                <div class="flex items-center justify-between gap-2">
                  <div class="min-w-0">
                    <p class="text-sm font-medium text-base-content/60 truncate">{{ skill.name }}</p>
                    <p class="text-xs text-base-content/40 font-mono truncate">{{ skill.slug }}</p>
                  </div>
        ...
        <div class="flex-1 min-h-0 overflow-y-auto">
          <div v-if="!selectedSkill" class="flex flex-col items-center justify-center h-full text-center px-8">
            ...
            <p class="text-sm font-medium text-base-content/60 mb-1">Selecciona un skill</p>
            <p class="text-xs text-base-content/40">Haz clic en un skill de la lista para ver sus detalles</p>
          </div>
          ...
          <div v-else class="p-8 max-w-3xl">
            ...
            <div class="flex items-start justify-between mb-6 gap-4">
              <div>
                <div class="flex items-center gap-3 mb-1">
                  <h2 class="text-xl font-bold text-base-content">{{ selectedSkill.name }}</h2>
                  <span
                    class="text-xs px-2 py-0.5 rounded-full font-medium"
                    :class="selectedSkill.isActive ? 'bg-emerald-500/10 text-emerald-400' : 'bg-base-100/60 text-base-content/50'">
                ...
                <p v-if="selectedSkill.description" class="text-sm text-base-content/60 mt-2">{{ selectedSkill.description }}</p>
              </div>
            ...
            <div class="rounded-xl border border-base-300 bg-base-300/50 overflow-hidden">
              <div class="px-4 py-2.5 border-b border-base-300 flex items-center gap-2">
                ...
                <span class="text-xs font-medium text-base-content/50">Contenido</span>
              </div>
              <pre class="p-4 text-sm text-base-content font-mono whitespace-pre-wrap leading-relaxed overflow-x-auto">{{ selectedSkill.content || '(sin contenido)' }}</pre>
            </div>
            ...
            <div class="mt-4 grid grid-cols-2 gap-3">
              <div class="rounded-lg border border-base-300 bg-base-300/30 px-4 py-3">
                ...
                <p class="text-xs text-base-content">{{ new Date(selectedSkill.createdAt).toLocaleString() }}</p>
              </div>
              <div class="rounded-lg border border-base-300 bg-base-300/30 px-4 py-3">
                <p class="text-xs text-base-content/50 mb-0.5">Actualizado</p>
                <p class="text-xs text-base-content">{{ new Date(selectedSkill.updatedAt).toLocaleString() }}</p>
              </div>
            </div>
          </div>
        </div>
    ...
    <AppModal
      ...
      <div class="px-6 py-5 space-y-5">
        <!-- Name -->
        <div>
          <label class="block text-xs font-medium text-base-content/60 mb-1.5">Nombre <span class="text-red-400">*</span></label>
          <input
            v-model="form.name"
            type="text"
            placeholder="Mi skill de análisis"
            class="w-full bg-base-200 border border-base-300 rounded-lg px-3 py-2 text-sm text-base-content placeholder:text-base-content/40 focus:outline-none focus:border-fuchsia-500 transition-colors" />
        </div>

        <!-- Slug -->
        <div>
          <label class="block text-xs font-medium text-base-content/60 mb-1.5">Slug <span class="text-red-400">*</span></label>
          <input
            v-model="form.slug"
            type="text"
            placeholder="mi-skill-de-analisis"
            class="w-full bg-base-200 border border-base-300 rounded-lg px-3 py-2 text-sm text-fuchsia-300 font-mono placeholder:text-base-content/40 focus:outline-none focus:border-fuchsia-500 transition-colors"
            @input="slugManuallyEdited = true" />
          <p class="text-xs text-base-content/40 mt-1">Solo minúsculas, números y guiones</p>
        </div>
        ...
        <!-- Content -->
        <div>
          <label class="block text-xs font-medium text-base-content/60 mb-1.5">Contenido (markdown)</label>
          <textarea
            v-model="form.content"
            rows="10"
            placeholder="# Skill&#10;&#10;Instrucciones del skill en formato markdown..."
            class="w-full bg-base-200 border border-base-300 rounded-lg px-3 py-2 text-sm text-base-content font-mono placeholder:text-base-content/40 focus:outline-none focus:border-fuchsia-500 transition-colors resize-y" />
        </div>
        ...
        <div v-if="editingSkill" class="flex items-center gap-3">
          <button
            ...
            :class="form.isActive ? 'bg-fuchsia-600' : 'bg-base-100'"
            @click="form.isActive = !form.isActive">
            <span
              class="absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-base-100 shadow transition-transform duration-200"
