            ...
            <tr v-for="user in users" :key="user.id" class="hover:bg-slate-800 transition-colors cursor-pointer"
              ...
              <td class="px-6 py-4">
                <div class="flex flex-wrap gap-1.5">
                  <span v-for="role in (user.roles ?? []).slice(0, 3)" :key="role.id"
        ...
        <form class="px-6 py-5 space-y-4" @submit.prevent="saveUser">
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input v-model="userForm.firstName" type="text" placeholder="Optional"
                class="w-full px-3 py-2.5 rounded-lg border border-slate-700 text-sm text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" />
            </div>
            <div>
          ...
          <div>
            ...
            <input v-model="userForm.username" type="text" placeholder="username" required :disabled="!!editingUser"
              class="w-full px-3 py-2.5 rounded-lg border border-slate-700 text-sm text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 disabled:bg-slate-50 disabled:text-slate-500" />
          </div>
          <div>
            <label class="block text-sm font-medium text-slate-400 mb-1.5">Email <span
                class="text-red-500">*</span></label>
          ...
          <div class="flex gap-3 pt-2">
            <button type="button"
              class="flex-1 px-4 py-2.5 rounded-lg border border-slate-700 text-sm font-medium text-slate-400 hover:bg-slate-50 transition-colors"
              @click="closeUserModal">
              Cancel
            </button>
            ...
            <div v-for="role in allRoles" :key="role.id"
              ...
              <div class="relative">
                <input type="checkbox" :checked="userHasRole(role.id)" :disabled="togglingRole === role.id"
                  ...
                  @click.stop @change="toggleRole(role.id)" />
              </div>
              <div class="flex-1">
