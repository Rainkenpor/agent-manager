...
<template>
  <PageLayout title="Integraciones"
    description="Asistente de chat embebible en sitios externos. Cada sitio se identifica por su origen; los nuevos se registran automáticamente al primer uso y quedan pendientes de configuración.">
    <template #actions>
        ...
        <div class="flex items-start justify-between gap-4">
          <div class="flex-1 min-w-0">
            ...
            <div class="flex flex-wrap gap-2 mt-2 text-xs">
              ...
              <span v-for="s in item.scope" :key="s" class="bg-base-200 text-base-content/70 px-2 py-0.5 rounded">{{
                scopeLabel(s) }}</span>
            </div>

            <!-- Embed snippet -->
            <div class="mt-3 flex items-center gap-2 text-xs">
              <code class="bg-base-200 px-2 py-1 rounded text-base-content/80 truncate flex-1">{{ embedSnippet(item)
              }}</code>
    ...
    <AppModal v-if="showModal" size="2xl" :title="editing ? 'Configurar integración' : 'Nueva integración'"
      ...
      <div class="px-6 py-5 space-y-5">
        ...
        <div>
          ...
          <p class="text-xs text-base-content/50 mt-1">Esquema + dominio del sitio anfitrión. No editable después de
            crear.</p>
        </div>
        ...
        <div>
          ...
          <div class="flex flex-wrap gap-2">
            <button v-for="opt in SCOPE_OPTIONS" :key="opt.key" type="button" @click="toggleScope(opt.key)"
              class="px-3 py-1.5 rounded-lg border text-sm transition-colors" :class="form.scope.includes(opt.key)
