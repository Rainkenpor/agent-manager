...
<template>
  <div class="p-6 text-base-content min-h-full">
            ...
            <div class="mt-3 space-y-1.5">
              ...
              <div v-if="item.authEnabled && item.secret" class="flex items-center gap-2 text-xs">
                ...
                <button @click="toggleSecret(item.id)" :title="revealedSecrets.has(item.id) ? 'Ocultar' : 'Mostrar'"
                  class="text-base-content/50 hover:text-base-content transition-colors">
                  <span class="mdi" :class="revealedSecrets.has(item.id) ? 'mdi-eye-off-outline' : 'mdi-eye-outline'"></span>
                </button>
    ...
    <!-- ── Modal ──────────────────────────────────────────────────────────── -->
    <AppModal v-if="showModal" size="2xl" :title="editing ? 'Editar Webhook' : 'Nuevo Webhook'" @close="showModal = false">
      <div class="px-6 py-5 space-y-5">
        ...
        <fieldset class="border border-base-300 rounded-xl p-4 space-y-3">
          ...
          <template v-else>
            <div>
              <label class="block text-sm text-base-content mb-1">Servidor MCP <span class="text-red-400">*</span></label>
