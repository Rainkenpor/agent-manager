...
<template>
  <div class="p-6 bg-slate-950 text-white min-h-full">
    ...
    <!-- Empty state -->
    <div
      v-else-if="listeners.length === 0"
      class="flex flex-col items-center gap-4 py-16 text-slate-500"
    >
    ...
    <div v-else class="space-y-3">
      <div
        ...
        :key="item.id"
        class="bg-slate-900 border border-slate-700/60 rounded-xl p-5"
      >
        <div class="flex items-start justify-between gap-4">
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-3 mb-1">
              <span
                ...
                :class="item.enabled ? 'bg-emerald-400' : 'bg-slate-600'"
              ></span>
          ...
          <div class="flex items-center gap-2 shrink-0">
            ...
            >
              <span class="mdi text-lg" :class="item.enabled ? 'mdi-toggle-switch' : 'mdi-toggle-switch-off-outline'"></span>
            </button>
            ...
            >
              <span
                class="mdi text-lg"
                :class="triggering === item.id ? 'mdi-loading mdi-spin' : 'mdi-play-circle-outline'"
              ></span>
            </button>
            ...
            >
              <span
                class="mdi text-lg"
                :class="deleting === item.id ? 'mdi-loading mdi-spin' : 'mdi-trash-can-outline'"
              ></span>
            </button>
          </div>
        </div>
    ...
    <Teleport to="body">
      ...
      >
        <div class="bg-slate-900 border border-slate-700/60 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
          <!-- Modal header -->
          <div class="flex items-center justify-between px-6 py-4 border-b border-slate-700/60">
            <h2 class="text-lg font-semibold text-white">
              {{ editing ? 'Editar Listener' : 'Nuevo Event Listener' }}
            </h2>
          ...
          <div class="px-6 py-5 space-y-5">
            ...
            <div>
              ...
              <input
                v-model="form.name"
                type="text"
                placeholder="Verificador de pedidos completados"
            ...
            <fieldset class="border border-slate-700 rounded-xl p-4 space-y-3">
              ...
              <div>
                ...
                <input
                  v-model="form.source.function_name"
                  type="text"
                  placeholder="mcp__erp__get_orders"
                  class="w-full bg-slate-800 border border-slate-600 rounded-lg px-3 py-2 text-white font-mono text-sm focus:outline-none focus:border-indigo-500"
                />
              </div>
              <div>
                ...
                <textarea
                  v-model="form.source.params"
                  rows="3"
                  placeholder='{ "limit": 50 }'
                  class="w-full bg-slate-800 border border-slate-600 rounded-lg px-3 py-2 text-white font-mono text-sm focus:outline-none focus:border-indigo-500 resize-none"
                ></textarea>
              </div>
            </fieldset>
            ...
            <fieldset class="border border-slate-700 rounded-xl p-4 space-y-3">
              ...
              <div>
                ...
                <input
                  v-model="form.condition.field"
                  type="text"
                  placeholder="element.status.name"
                  class="w-full bg-slate-800 border border-slate-600 rounded-lg px-3 py-2 text-white font-mono text-sm focus:outline-none focus:border-indigo-500"
                />
              ...
              <div class="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    type="text"
                    placeholder='"Completado" o 42 o true'
                    class="w-full bg-slate-800 border border-slate-600 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-indigo-500"
                  />
                </div>
              </div>
            ...
            <fieldset class="border border-slate-700 rounded-xl p-4 space-y-4">
              ...
              <div
                v-for="(action, idx) in form.action"
                :key="idx"
              ...
              >
                ...
                <div>
                  ...
                  <input
                    v-model="action.function_name"
                    type="text"
                    placeholder="mcp__slack__send_message"
                    class="w-full bg-slate-800 border border-slate-600 rounded-lg px-3 py-2 text-white font-mono text-sm focus:outline-none focus:border-indigo-500"
                  />
                </div>
                <div>
                  ...
                  <textarea
                    ...
                    rows="3"
                    placeholder='{ "template": "shipping_alert" }'
                    class="w-full bg-slate-800 border border-slate-600 rounded-lg px-3 py-2 text-white font-mono text-sm focus:outline-none focus:border-indigo-500 resize-none"
                  ></textarea>
                </div>
              </div>
