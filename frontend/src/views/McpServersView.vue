...
<template>
  <PageLayout title="MCP Servers" description="Manage external Model Context Protocol servers">
    <template #actions>
      <button
        class="flex items-center gap-2  hover:bg-indigo-700 text-white px-4 py-2.5 rounded-lg text-sm font-medium transition-colors shadow-sm"
        @click="openCreate">
    ...
    <div v-else>
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
        <Card v-for="server in servers" :key="server.id">
          ...
          <template v-if="server.type !== 'local'" #options>
            <button class="btn w-full btn-ghost justify-start" @click="confirmDelete(server)">
              <span class="mdi mdi-delete"></span>
              Delete
            </button>
            <button class="btn w-full btn-ghost justify-start" @click="openEdit(server)">
      ...
      <div
        ...
        <div class="px-6 py-4 border-t border-slate-700 flex gap-3 shrink-0">
          ...
          <button
            ...
            :disabled="callRunning" @click="executeTool">
  ...
  <!-- Create / Edit Modal -->
  <AppModal v-if="showModal" size="lg" :title="editingServer ? 'Edit MCP Server' : 'Add MCP Server'"
    @close="closeModal">
    <form id="mcp-server-form" class="px-6 py-5 space-y-4" @submit.prevent="saveServer">
      <!-- Name + Display Name -->
