...
<template>
  <PageLayout title="Credenciales MCP"
    description="Claves y tokens personales por servidor MCP. El agente puede acceder a estos valores durante las conversaciones."
  ...
  <AppModal v-if="showForm" @close="() => { showForm = false; formEditing = null }"
    :title="formEditing ? 'Editar credencial' : 'Nueva credencial'">
    <NewCredential :servers="servers" :editing="formEditing" @saved="onCredentialSaved"
      @cancel="() => { showForm = false; formEditing = null }" />
  </AppModal>


  <!-- Delete Confirm -->
  <ConfirmDialog v-if="deleteTarget" title="Eliminar credencial"
