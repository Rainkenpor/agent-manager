...
<template>
  <div class="flex h-full  text-white overflow-hidden">
        ...
        <button v-for="server in servers" :key="server.id" @click="selectServer(server)"
          ...
          <div class="flex items-start justify-between gap-2">
            ...
            <div class="flex items-center gap-1 shrink-0">
              <span class="inline-block w-2 h-2 rounded-full mt-0.5"
                :class="server.active ? 'bg-emerald-500' : 'bg-slate-600'"></span>
            </div>
          </div>
          ...
          <div class="flex gap-1 mt-2" @click.stop>
            ...
            <button v-if="auth.hasPermission('hook_servers', 'delete')" @click="deleteServer(server)"
              class="p-1 rounded text-slate-500 hover:text-red-400 hover:bg-slate-800 transition-colors" title="Delete">
      ...
      <template v-else>
        ...
        <div class="w-72 shrink-0 flex flex-col border-r border-slate-800">
          ...
          <div class="flex-1 overflow-y-auto p-3 space-y-1.5">
            ...
            <div v-else-if="discoveredHooks.length === 0" class="text-center py-8 text-slate-500 text-sm">No hooks found
            </div>
          ...
          <template v-else>
            <div class="px-6 py-4 border-b border-slate-800 flex items-start justify-between gap-4">
              <div>
                ...
                <p v-if="selectedHook.description" class="text-sm text-slate-400 mt-1">{{ selectedHook.description }}
                </p>
              </div>
                    ...
                    <tbody>
                      <tr v-for="(field, fieldName) in selectedHook.payload" :key="fieldName"
                        ...
                        <td class="px-4 py-2.5">
                          <span class="px-1.5 py-0.5 rounded bg-slate-800 text-indigo-400 text-xs font-mono">{{
                            field.type }}</span>
                        </td>
      ...
      <div class="relative z-10 w-full max-w-md bg-slate-900 rounded-xl border border-slate-700 shadow-2xl">
        ...
        <form @submit.prevent="saveServer" class="p-6 space-y-4">
          <div v-if="!editingServer">
            <label class="block text-xs font-medium text-slate-400 mb-1.5">Name <span
          ...
          <div>
            <label class="block text-xs font-medium text-slate-400 mb-1.5">Base URL <span
                class="text-red-400">*</span></label>
            <input v-model="serverForm.url" required type="url" placeholder="http://localhost:4000/event-source"
              class="w-full px-3 py-2 rounded-lg bg-slate-800 border border-slate-700 text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:border-indigo-500" />
            <p class="text-xs text-slate-500 mt-1">The server must expose <code class="text-indigo-400">/hooks</code>
              and
          ...
          <div>
            ...
            <textarea v-model="serverForm.description" rows="2" placeholder="Optional description..."
              class="w-full px-3 py-2 rounded-lg bg-slate-800 border border-slate-700 text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:border-indigo-500 resize-none"></textarea>
          </div>
  ...
  <Teleport to="body">
    <div v-if="showAssignModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div
        class="relative z-10 w-full max-w-2xl bg-slate-900 rounded-xl border border-slate-700 shadow-2xl flex flex-col max-h-[85vh]">
        <!-- Header -->
        ...
        <div class="flex-1 overflow-hidden grid grid-cols-2 divide-x divide-slate-800">
          ...
          <div class="flex flex-col overflow-hidden">
            ...
            <div class="flex-1 overflow-y-auto">
              <div v-if="agents.length === 0" class="px-4 py-6 text-sm text-slate-500 italic text-center">No agents
                available</div>
          ...
          <div class="flex flex-col overflow-hidden">
            ...
            <div class="flex-1 overflow-y-auto">
              <div v-if="!selectedMcpServerId" class="px-4 py-6 text-sm text-slate-500 italic text-center">Select a
                server
                above</div>
              <div v-else-if="loadingMcpTools" class="px-4 py-6 text-sm text-slate-500 text-center">Loading tools...
              </div>
              <div v-else-if="mcpToolsForServer.length === 0"
                class="px-4 py-6 text-sm text-slate-500 italic text-center">
