...
<template>
  <div
    class="disti-root"
    :class="[`disti-size-${size}`, `disti-state-${state}`, theme === 'dark' ? 'disti-dark' : '']"
  >
    <!-- Logo stage -->
    <div class="disti-stage" :style="stageStyle">
      <span
        ...
        :key="b.i"
        class="disti-block"
        :class="b.layer === 0 ? 'disti-back' : 'disti-front'"
        :style="blockStyle(b)"
      />
    </div>
...
/* ── Tamaños ── */
.disti-size-xs { --cell: 6px;  gap: 0; }
.disti-size-sm { --cell: 10px; gap: 4px; }
.disti-size-md { --cell: 16px; gap: 7px; }
.disti-size-lg { --cell: 24px; gap: 10px; }

/* xs: solo el logo, sin label, sin sombra excesiva */
.disti-size-xs .disti-stage {
  filter: drop-shadow(0 2px 6px rgba(58, 43, 138, 0.3));
}
.disti-size-xs .disti-label { display: none; }
...
.disti-state-sad .disti-back  { background: linear-gradient(135deg, #8a96b0, #6b7794); }
.disti-state-sad .disti-front { background: linear-gradient(135deg, #6b7794, #8a96b0); }
.disti-state-sad .disti-stage { filter: drop-shadow(0 4px 8px rgba(40, 50, 80, 0.15)) saturate(0.6); }
...
.disti-dark .disti-label { color: #c9d4ec; }
.disti-state-sad .disti-label { color: #6b7794; }

.disti-emoji { margin-right: 2px; }
...
.disti-dots i:nth-child(2) { animation-delay: 0.15s; }
.disti-dots i:nth-child(3) { animation-delay: 0.30s; }
...
@keyframes d-build {
  ...
  85%  { opacity: 0; transform: translate(calc(var(--dx) * -0.6), calc(var(--dy) * -0.6)) scale(0.2); }
  100% { opacity: 0; transform: translate(calc(var(--dx) * -0.6), calc(var(--dy) * -0.6)) scale(0.2); }
}
@keyframes d-think {
...
@keyframes d-excited {
  ...
  20%      { transform: translate(-1px, -2px) scale(1.10); }
  40%      { transform: translate(2px, 1px)   scale(0.94); }
  60%      { transform: translate(-1px, 2px)  scale(1.08); }
  80%      { transform: translate(1px, -1px)  scale(0.96); }
}
@keyframes d-sad {
  0%, 100% { transform: translateY(0)   scale(1); }
...
@media (prefers-reduced-motion: reduce) {
  .disti-block { animation: none !important; opacity: 1 !important; transform: none !important; }
}
</style>
