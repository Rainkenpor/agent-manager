...
import type { GeneratePresetQnaUseCase } from '../preset-qna/generate-preset-qna.use-case.js'
import type { MatchPresetQnaUseCase } from '../preset-qna/match-preset-qna.use-case.js'
...
/**
 * Responde un mensaje del widget de integración igual que el asistente público:
 * primero intenta una respuesta preestablecida y, si no hay coincidencia, invoca al agente.
 * El `extraContext` (origen + scope) se inyecta como contexto del agente.
 */
export class IntegrationChatAnswerUseCase {
	constructor(
	...
	async execute(conversationId: string, content: string, sendEvent: SendEvent, signal?: AbortSignal, extraContext?: string): Promise<void> {
		...
		await this.streamMessageUseCase.execute(conversationId, content, capturing as never, signal, extraContext)
