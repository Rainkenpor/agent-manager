...
function jsonSchemaPropertyToZod(prop: Record<string, unknown>, required: boolean): ZodTypeAny {
	...
	if (type === 'string' || (!type && !prop.properties && !prop.items)) {
		...
		const itemSchema = items ? jsonSchemaPropertyToZod(items, true) : z.unknown()
...
function jsonSchemaToZodShape(jsonSchema: Record<string, unknown>): ZodRawShape {
...
async function applyRoleBasedTools(server: McpServer, user: Record<string, unknown>, sessionId: string): Promise<void> {
	...
	const agentList: Array<{ id: string; name: string; slug: string; description: string }> = []
	...
	for (const agent of agentList) {
		server.tool(
			`agent_${agent.slug}`,
			`Platform agent: ${agent.name}. ${agent.description}`,
			{
...
/**
 * Register a local tool (MCP server)
 */
function registerLocalTool(server: McpServer, route: RegisteredRoute, sessionId: string) {
...
const mcpAuthMiddleware = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
	...
	const mcpToken = req.headers['x-mcp-token'] || req.headers['mcp-token'] || req.query.token || req.query.mcp_token
	...
	if (!authHeader?.startsWith('Bearer ')) {
		const serverPortAuth = process.env.SERVER_PORT_AUTH || ''
...
export function registerMCPOauthRoutes(oauthService: McpOAuthService): express.Router {
	const router = express.Router()

	for (const route of registry.getRoutes().filter((r) => r.useBy.includes('server'))) {
		const expressMethod = route.method.toLowerCase() as 'get' | 'post' | 'put' | 'delete' | 'patch'
