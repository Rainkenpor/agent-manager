 ...
 */
export function registerServerRoutes(oauthService?: McpOAuthService): express.Router {
	...
	for (const route of registry.getRoutes().filter((r) => r.useBy.includes('server'))) {
		...
		if (route.requiredPermission) {
			middlewares.push(requirePermission(route.requiredPermission.resource, route.requiredPermission.action))
		}
			...
			try {
				...
				if (!parseResult.success) {
					...
					return res.status(400).json({
						...
						details: Object.entries(error.fieldErrors).map(([key, value]) => `${key}: ${value}`),
	...
	console.log(`🌐 API Router created with ${registry.getRoutes().length} routes`)
