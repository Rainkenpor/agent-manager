		...
		handler: async ({ context: { req, res }, oauthService }) => {
			...
			const UI_BASE_PATH = process.env.UI_BASE_PATH
				? process.env.UI_BASE_PATH.endsWith('/')
					...
					: `${process.env.UI_BASE_PATH}/`
				: '/'
			const apiBase = `${req.protocol}://${req.get('host')}${serverPortAuth}${UI_BASE_PATH}`
			const mcpBase = `${req.protocol}://${req.hostname}${serverPortAuth}${UI_BASE_PATH}`
	...
	registry.register({
		...
		handler: async ({ input, context: { req, res }, oauthService }) => {
			...
			if (token) {
				...
				user = (await container.userRepository.findByUsername(username)) ?? (await container.userRepository.findByEmail(username))
				if (!user) {
					return { error: 'invalid_credentials' }
				}
			...
			const code = await oauthService.createAuthorizationCode(client_id, redirect_uri, user.id, scope)
