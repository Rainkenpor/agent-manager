...
export class RouteToolRegistry {
	...
	register<T extends ZodRawShape | z.ZodObject<any>>(
		config: RouteToolConfig<T>,
	): void {
		this.routes.push({
	...
	applyToMcpServer({
		...
		for (const route of routes) {
			if (!route.toolName || !route.toolDescription || !route.inputSchema) {
				console.warn(
					"Route is missing toolName, toolDescription or inputSchema:",
					route.method,
					route.path,
				);
				continue;
			...
			const inputSchema =
				route.inputSchema instanceof z.ZodObject
					? route.inputSchema
					: z.object(route.inputSchema as ZodRawShape);
