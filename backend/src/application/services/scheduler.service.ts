...
export class ServiceScheduler {
	...
	register(service: ScheduledService): this {
		if (this.services.has(service.name)) {
			console.warn(`⚠️  Service "${service.name}" is already registered. Replacing...`)
		...
		console.log(`📝 Registered scheduled service: "${service.name}" (interval: ${service.interval}ms)`)
	...
	startAll(): void {
		...
		console.log(`✅ Scheduler started with ${this.timers.size} active service(s)`)
	}
	 ...
	 */
	getStatus(): Record<string, { enabled: boolean; running: boolean; interval?: number }> {
		const status: Record<string, { enabled: boolean; running: boolean; interval?: number }> = {}
