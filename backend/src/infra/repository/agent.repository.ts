...
export class AgentRepository implements IAgentRepository {
	...
	async findAll(options?: {
		...
		const rows = await db
			...
			.from(agents)
			.where(
				options?.useByChat !== undefined
					? eq(agents.useByChat, options.useByChat)
					: undefined,
			)
	...
	async update(data: UpdateAgentDTO): Promise<AgentWithSubagents | undefined> {
		...
		if (data.temperature !== undefined)
			updateValues.temperature = data.temperature;
	...
	async getSubagents(agentId: string): Promise<AgentRecord[]> {
		const relations = await db
			...
			.orderBy(asc(agentSubagents.order));
